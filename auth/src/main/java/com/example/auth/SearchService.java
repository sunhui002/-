...
import com.example.auth.Entity.SearchResult;
...
public class SearchService {
    ...
    private SearchRequest bulidSearchRequest(SearchParam searchParam) {
        ...
        if (!StringUtils.isEmpty(searchParam.getKeyword())) {
            boolQueryBuilder.must(QueryBuilders.matchQuery("spuTitle", searchParam.getKeyword()));
        }
...
//        }
        //1.2.4 priceRange
        RangeQueryBuilder rangeQueryBuilder = QueryBuilders.rangeQuery("spuPrice");
        ...
        if (attrs!=null&&attrs.size() > 0) {
            attrs.forEach(attr->{
                ...
                queryBuilder.must(QueryBuilders.termQuery("attrs.attrname", attrSplit[0]));
        ...
        if (!StringUtils.isEmpty(searchParam.getKeyword())) {
            ...
            highlightBuilder.field("spuTitle");
...
//        attrIdAgg.subAggregation(attrValueAgg);
        ...
        nestedAggregationBuilder.subAggregation(attrNameAgg);
    ...
    private SearchResult bulidSearchResult(SearchParam searchParam, SearchResponse searchResponse) {
        ...
        if (hits.getHits()!=null&&hits.getHits().length>0){
            ...
            for (SearchHit hit : hits) {
                ...
                EsProduct esproduct = JSON.parseObject(sourceAsString, EsProduct.class);
                //设置高亮属性
                if (!StringUtils.isEmpty(searchParam.getKeyword())) {
                    HighlightField skuTitle = hit.getHighlightFields().get("spuTitle");
                    String highLight = skuTitle.getFragments()[0].string();
                    esproduct.setSpuTitle(highLight);
                }
                esproducts.add(esproduct);
            }
            result.setProduct(esproducts);
        }
...
//        result.setCatalogs(catalogVos);
        ...
        ParsedStringTerms attrNameAgg=parsedNested.getAggregations().get("attrNameAgg");
//        ParsedLongTerms attrValueAgg=parsedNested.getAggregations().get("attrValueAgg");
